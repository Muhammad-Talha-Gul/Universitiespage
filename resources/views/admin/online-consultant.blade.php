            ...
            <div class="card-box table-responsive">
                ...
                <table id="datatable" class="table hover">
                    ...
                    <tr>
                        ...
                        <td>{{$value->student_phone_number}}</td>
                        <td>{{$value->student_country}}</td>
                        <td>{{$value->student_state}}</td>
                        <td>{{$value->student_city}}</td>
                        <td>{{date('dS M, Y h:i a',strtotime($value->created_at))}}</td>
                        <td>
                            @if(check_access(Auth::user()->id,'student','_show')==1)<a href="{{ route('consultant-details', ['id' =>$value->id]) }}" class="btn btn-info btn-xs"><i class="fa fa-eye"></i></a>
                            @endif
                            @if(check_access(Auth::user()->id,'student','_show')==1)<a href="{{ route('consultant-delete', ['id' =>$value->id]) }}" class="btn btn-info btn-xs"><i class="fa fa-trash"></i></a>
                            @endif
