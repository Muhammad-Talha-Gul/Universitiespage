		...
		<div class="col-md-12">
			<div class="card-box table-responsive">
                ...
                <div class="form-group pull-right" style="margin-bottom: 10px;">
                    @if(check_access(Auth::user()->id,'sliders','_delete')==1)
                    <form action="{{route("deleteSliders")}}" method="POST" id="del_form" form="del_form">{{csrf_field()}}</form>
                    <a class="btn btn-danger btn-sm" id="deleteAll">Delete (<span id="checkCount">0</span>)</a>
                    @endif
                    @if(check_access(Auth::user()->id,'sliders','_create')==1)
                    <a href="{{route("addSlider")}}" class="btn btn-primary btn-sm">Add Slider</a>
                    @endif
                </div>
                <h4 class="m-t-0 header-title"><b>Sliders List</b></h4>
                ...
                <table id="datatable" class="table hover">
                	...
                	<tr>
                        ...
                        @if(check_access(Auth::user()->id,'sliders','_edit')==1)
                		<td><a href="{{route('editSlider',$value->id)}}" class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i></a></td>
                        @endif
                	</tr>
                	@endforeach
