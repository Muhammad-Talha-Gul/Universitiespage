...
class HomeController extends Controller
{
    ...
    public function emails($type)
    { 
        $data = ContactMails::WHERE('type',$type)->OrderBy('id','DESC')->paginate();
        return view('admin.emails',compact('data'));
    }
    
        public function courseslog()
    {
        $data = ContactButton::OrderBy('id','DESC')->where('university_id', 0)->paginate();
        $user = User::OrderBy('id','DESC')->get();
        $courses = Course::OrderBy('id','DESC')->get();
        return view('admin.courses_emails',array('data' => $data, 'users' => $user, 'courses' => $courses));

    }
    
        public function universitylog()
    {
        $data = ContactButton::OrderBy('id','DESC')->where('course_id', 0)->paginate();
        $user = User::OrderBy('id','DESC')->get();
        $universities = UniversityDetail::OrderBy('id','DESC')->get();
        return view('admin.universities_emails',array('data' => $data, 'users' => $user, 'universities' => $universities));
