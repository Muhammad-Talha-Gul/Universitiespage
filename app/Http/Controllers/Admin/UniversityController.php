...
class UniversityController extends Controller
{
    ...
    {
        $unvnamee = $request->input('first_name');
        $unvnameefullunv = $request->input('university_name');
        $emailalertnative = $request->input('email');
        ...
        $request['email'] = $emailalertnative;
        ...
        $notificationrun = Notifications::create([
            'type'=>'account',
            ...
            'is_read'=>0,
                     ...
                     UniversityDetail::where('name', $unvnameefullunv)->update(array('alternate_email' => $emailalertnative));
    ...
    { 
        $unvnamee = $request->input('university_name');
        $emailalertnative = $request->input('email');
    	$this->validate($request, [
            'first_name' => 'required|string',
        ...
        UniversityDetail::where('name', $unvnamee)->update(array('alternate_email' => $emailalertnative));
    ...
    public function course(Request $request){
        ...
        elseif(isset($request->type) && $request->type=='course'){
            ...
            // $courses = Course::whereRaw($c_raw)->whereHas('university', function ($query) use ($location) {
            // $query->where('country', $location);})
            // ->with('university', 'qualificationName', 'subject')
            // ->orderBy('name')
            // ->paginate($request->limit ?? 10);
            $courses = Course::with('university', 'qualificationName', 'subject')
            ->whereRaw($c_raw)
            ->orderBy('name')
            ->paginate($request->limit ?? 10);
            ...
            $universities = UniversityDetail::with(['courses'=>function($que) use($c_raw){
                            ...
                            ->whereDate('deadline', '>=', date('Y-m-d'))->orderBy('id');
                            },
                            'courses.qualificationName'])->whereRaw($raw)
