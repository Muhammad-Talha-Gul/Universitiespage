...
class CourseController extends Controller
{
    ...
    {   
        $data = Course::with('university')->where('display',1)->orderBy('id', 'DESC')->paginate(100);
        return view('admin.course',['data'=>$data]);
    }
     public function courses($name = null){
         if (!empty($name)) {
            $data = Course::with(['university', 'subject'])->where('display', 1)->where('name', 'like', '%' . $name . '%')->orderBy('id', 'DESC')->paginate(100);
            return view('admin.course_partial', ['data' => $data])->render();
        }else {
            $data = Course::with(['university', 'subject'])->where('display', 1)->orderBy('id', 'DESC')->paginate(100);
            return view('admin.course_partial',['data'=>$data]);
        }
    }
